...
public class 健康保険法読込 extends Parser {
    ...
    public static Type 数字 = new Type("数字", "(?<H>" + P数字 + ")条" + P空白 + "(?<T>.*)", KAN_ID);
    public static Type 漢数字 = new Type("漢数ハ", "(?<H>" + P漢数字 + ")" + P空白 + "(?<T>.*)", KAN_ID);
    public static Type イロハ = new Type("イロハ", "(?<H>" + Pイロハ + ")" + P空白 + "(?<T>.*)", KAN_ID);
